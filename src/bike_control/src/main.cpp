...
public:
                ...
                motor_(serial_port_)  
    {
        ...
        // 声明控制器参数
        this->declare_parameter<double>("wheelbase_", 1.0);
        this->declare_parameter<double>("max_speed_", 1.0);
        this->declare_parameter<double>("min_speed_", 0.0);
        this->declare_parameter<double>("max_steer_", 30.0);
        this->declare_parameter<double>("max_accel_add", 1.0);
        this->declare_parameter<double>("max_accel_red", 1.0);
        this->declare_parameter<double>("max_dyaw_rate_", 30.0);
        this->declare_parameter<double>("predict_time_", 3.5);
        this->declare_parameter<double>("goal_cost_gain_", 0.1);
        this->declare_parameter<double>("speed_cost_gain_", 0.2);
        this->declare_parameter<double>("obstacle_cost_gain_", 0.55);
        this->declare_parameter<double>("to_goal_cost_gain_", 0.05);
        this->declare_parameter<double>("max_steer_rate_", 30.0);
        this->declare_parameter<double>("safety_margin_", 0.5);
        this->declare_parameter<double>("OBSTACLE_INFLUENCE_DIST", 0.8);
...
private:
	void initLogFile() {
	    // 设置固定日志路径
	    fs::path full_log_path = "/home/sunrise/Ebike_Human_Follower/logs";
    ...
    void executeControlCycle() {
        ...
        // bike_f_yaw_ = (rotor_state.position / 10) * (M_PI / 180.0);
        ...
        if (motor_cmd.hub_throttle <=0) {
            ...
            if(first_up)
            {
                ...
                motor_.controlHubMotor(3, motor_cmd.hub_throttle, 0);
                first_up = false;
            }
            else
            {
        ...
        // 更新可视化
        visualizer_->update(
            state.front_x, state.front_y, state.front_yaw,
